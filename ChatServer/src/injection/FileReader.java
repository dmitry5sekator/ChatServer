...
public class FileReader extends RouteReader 
{
	...
	public void read(ApplicationController ap) 
	{
		...
		try
		{
			String patt [] = {"(POST /chatroom/member/ HTTP/1.1)","(POST /chatroom/ HTTP/1.1)","(POST /chatroom/message/ HTTP/1.1)","(GET /chatroom/)[\\d]{1,6}(/messages/ HTTP/1.1)","(POST /users/signIn/ HTTP/1.1)","(POST /users/ HTTP/1.1)","(DELETE /chatroom/)[\\d]{1,6}(/member/)[\\d]{1,6}(/ HTTP/1.1)","(GET /logout/)[\\d]{1,6}(/room/)[\\d]{1,6}(/ HTTP/1.1)"};
